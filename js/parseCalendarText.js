...
function parseCalendarText(txt, options) {
  ...
  var warn = function (message) {
    return warnings.push({ message: message });
  };
  ...
  if (options.filepath) {
    ...
    if (filepathDate && isValidDate_1.default(filepathDate)) day = filepathDate;
  }
  try {
    // loop through lines
    for (
      var _b = __values(util_1.splitLines(txt)), _c = _b.next();
      !_c.done;
      _c = _b.next()
    ) {
      var line = _c.value;
      var _d = __read(util_1.unindent(line), 2),
        message = _d[0],
        indentedness = _d[1];
      ...
      if (timeTag || durationTag) {
        var startTime =
          timeTag && timeTag.startTime ? timeTag.startTime : previousEndTime;
        var endTime = void 0;
        if (timeTag && timeTag.endTime) endTime = timeTag.endTime;
        else if (durationTag)
          endTime = HoursMinutes_1.add(startTime, durationTag);
        else
          endTime = HoursMinutes_1.add(
            startTime,
            config_1.default.defaultEventDuration
          );
        var newEvent = {
          startTime: startTime,
          endTime: endTime,
        ...
        if (
          parentEvent &&
          HoursMinutes_1.toMinutes(endTime) >
            HoursMinutes_1.toMinutes(parentEvent.endTime)
        ) {
          if (parentEvent.explicitEndTime || parentEvent.explicitDuration)
            warn("Oh no, a schedule overflow has occurred!");
          else parentEvent.endTime = __assign({}, endTime);
        }
      }
      // Append description
      if (parentEvent) {
        if (!parentEvent.description) parentEvent.description = "";
        parentEvent.description += util_1.decrementIndent(
          line,
          parentEvent.indentedness + config_1.default.tabWidth
        );
      }
    }
  } catch (e_1_1) {
...
function getParentEvent(events, indentedness) {
  ...
  for (var i = events.length - 1; i >= 0; --i) {
    if (events[i].indentedness < indentedness) return events[i];
  }
  // Otherwise,
  return null;
}
