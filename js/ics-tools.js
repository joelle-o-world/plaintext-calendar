...
exports.cancelActiveEvents =
  exports.readICSProperty =
  exports.insertEvents =
  exports.calendarWrap =
  exports.encodeICSEvent =
  exports.parseICSEvent =
  exports.extractICSEvents =
    void 0;
  ...
  return __generator(this, function (_a) {
    switch (_a.label) {
      ...
      case 1:
        ...
        return [
          4 /*yield*/,
          str.slice(result.index + "BEGIN:VEVENT".length, endIndex),
        ];
      case 2:
        _a.sent();
        return [3 /*break*/, 1];
...
function parseICSEvent(str) {
  ...
  try {
    for (
      ...
      if (out[key]) console.warn("duplicate keys in event", key);
      out[key] = value;
    }
  } catch (e_1_1) {
    e_1 = { error: e_1_1 };
...
function encodeICSEvent(ev, addWrappers) {
  if (addWrappers === void 0) {
    addWrappers = false;
  }
  var lines = [];
  for (var key in ev)
    if (typeof ev[key] === "string") lines.push(key + ":" + ev[key]);
  if (addWrappers) lines = __spread(["BEGIN:VEVENT"], lines, ["END:VEVENT"]);
  return lines.join("\r\n");
}
...
function cancelActiveEvents(str) {
  ...
  try {
    for (
      var _b = __values(extractICSEvents(str)), _c = _b.next();
      !_c.done;
      _c = _b.next()
    ) {
      var e = _c.value;
